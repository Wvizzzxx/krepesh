...
export function productRoutes(app: FastifyInstance): void {
  ...
  app.post('/api/products', { preHandler: [authenticate, adminOnly] }, async (request: FastifyRequest, reply) => {
    try {
  ...
  app.put('/api/products/:id', { preHandler: [authenticate, adminOnly] }, async (request: FastifyRequest, reply) => {
    try {
      const { id } = request.params as { id: string }
  ...
  app.delete('/api/products/:id', { preHandler: [authenticate, adminOnly] }, async (request: FastifyRequest, reply) => {
    try {
      const { id } = request.params as { id: string }
  ...
  app.post('/api/products/:id/images', { preHandler: [authenticate, adminOnly] }, async (request: FastifyRequest, reply) => {
    try {
      const { id } = request.params as { id: string }
      ...
      for await (const file of files) {
        const url = await app.saveUpload(file.file)
        imageUrls.push(url)
      }
      ...
      handleError(reply, 500, 'Failed to upload images')
    }
  })

  // Delete a specific image from product
  app.delete('/api/products/:id/images', { preHandler: [authenticate, adminOnly] }, async (request: FastifyRequest, reply) => {
    try {
      const { id } = request.params as { id: string }
