...
export function categoryRoutes(app: FastifyInstance): void {
  ...
  app.put('/api/categories/:id', { preHandler: [authenticate, adminOnly] }, async (request: FastifyRequest, reply) => {
    try {
      const { id } = request.params as { id: string }
  ...
  app.delete('/api/categories/:id', { preHandler: [authenticate, adminOnly] }, async (request: FastifyRequest, reply) => {
    try {
      const { id } = request.params as { id: string }
      ...
      if (category.image) {
        const localPath = path.join(__dirname, '..', category.image)
  ...
  app.post('/api/categories/:id/image', { preHandler: [authenticate, adminOnly] }, async (request: FastifyRequest, reply) => {
    try {
      const { id } = request.params as { id: string }
